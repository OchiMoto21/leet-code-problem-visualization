...
class DynamicProgramming extends Component {
    ...
    renderDPElements() {
        ...
        const answer_style = { borderBottom: "var(--border-color) 1px solid", margin: 0, padding: "0.5rem" };

        return Object.entries(stepMatrices).map(([step, matricesObject], index) => {
            if (Object.keys(matricesObject).length === 0) return "";
            if (step === "Final Answer") return (
                            ...
                            <table>
                                <tbody>
                                    <tr>
                                        ...
                                        {value.slice(matricesObject["Answer"][0], matricesObject["Answer"][1] + 1).split("").map((char, charIndex) => {

                                            return (
                                                <td key={char + charIndex}>
                                                    <h4 style={answer_style}>{char}</h4>

                                                    <h4 style={answer_style}>{matricesObject["Answer"][0] + charIndex}</h4>
                                                </td>
                                            )
                                        })}
                                    </tr>
            ...
            return (
                <div key={step + "[" + index + "]"}>
                    ...
                    {Object.entries(matricesObject).map(([matrixKey, matrix], matricesIndex) => (
                        <div key={matrixKey} className='step-container'>
